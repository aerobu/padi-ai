...
class TestConsentService:
    ...
    def test_initiate_consent(self, engine):
        ...
        with engine.connect() as conn:
            conn.execute(text("""
                ...
                VALUES (:parent_id, 'coppa_verifiable', 'pending', '192.168.1.1', 'Mozilla/5.0', 'sha256hash')
            """), {"parent_id": parent_id})
            conn.commit()

        with engine.connect() as conn:
            result = conn.execute(text("SELECT status FROM consent_records WHERE parent_id = :parent_id"), {"parent_id": parent_id}).fetchone()
    ...
    def test_confirm_consent(self, engine):
        ...
        # Confirm consent
        with engine.connect() as conn:
            conn.execute(text("UPDATE consent_records SET status = 'active', confirmed_at = CURRENT_TIMESTAMP WHERE parent_id = :parent_id"), {"parent_id": parent_id})
            conn.commit()
    ...
    def test_consent_expiration_check(self, engine):
        ...
        with engine.connect() as conn:
            conn.execute(text("""
                ...
                VALUES (:parent_id, 'coppa_verifiable', 'active', :expiry)
            """), {"parent_id": parent_id, "expiry": datetime.now(timezone.utc) + timedelta(days=365)})
            conn.commit()

        with engine.connect() as conn:
            result = conn.execute(text("SELECT expires_at > CURRENT_TIMESTAMP as is_active FROM consent_records WHERE parent_id = :parent_id"), {"parent_id": parent_id}).fetchone()
    ...
    def test_check_consent_required(self, engine):
        ...
        with engine.connect() as conn:
            result = conn.execute(text("""
                SELECT EXISTS (SELECT 1 FROM consent_records WHERE parent_id = :parent_id AND status = 'active') as has_consent
